(function initParser(root, factory) {
  ...
  const MODES = {
    withdrawal: { kind: "출금", sign: -1 },
    income: { kind: "입금", sign: 1 },
  };

  function parseBankRows(rows, options = {}) {
    const mode = MODES[options.mode] ? options.mode : "withdrawal";
    ...
    return rows
      ...
      .filter((row) => row && matchesMode(row, mode))
      .map((row) => {
        const uploadKey = makeUploadKey(row, mode);
        const duplicated = existingKeys.has(uploadKey);
        const selectedType = guessType(row, types, rules);
        return {
          ...row,
          mode,
          kind: MODES[mode].kind,
          uploadKey,
          duplicated,
          include: !duplicated,
          selectedType,
          uploadPayload: toLedgerPayload(row, selectedType, uploadKey, mode),
        };
      });
  }

  function matchesMode(row, mode) {
    if (mode === "income") return row.amount > 0;
    return row.amount < 0;
  }
  ...
  function toLedgerPayload(row, selectedType, uploadKey, mode = row.mode || "withdrawal") {
    return {
      kind: MODES[mode]?.kind || row.kind || "출금",
      month: row.month,
      date: row.date,
  ...
  function toUploadPayload(row, selectedType, uploadKey) {
    return toLedgerPayload(row, selectedType, uploadKey, "withdrawal");
  }
